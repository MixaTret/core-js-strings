...
function formatTime(minutes, seconds) {
  ...
  const str2 = seconds.toString().padStart(2, '0');
  return `${str1}:${str2}`;
}
...
function reverseString(str) {
  return Array.from(str).reverse().toString().replace(/,/g, '');
}
...
function findLongestWord(sentence) {
  const longestWord = sentence.split(' ').reduce(function (
    longest,
    currentWord
  ) {
...
function reverseWords(str) {
  ...
  return newStr.split(' ').reverse().toString().replace(/,/g, ' ');
}
...
function getCardId(value) {
  ...
  const result = comparisonStr.indexOf(value);
  return result;
}
